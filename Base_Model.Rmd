...
model_string <- "
model {
  # TRAINING PERIOD: Likelihood for observed data (learn parameters)
  for(i in 1:t_train) {
  ...
  # EVALUATION PERIOD: Generate predictions for held-out weeks
...
n_holdout <- 2  # Hold out the last 2 weeks
...
cat("Training period: 1 to", t_train, "\n")
cat("Evaluation period:", t_train + 1, "to", tmax, "\n")

# Prepare data list for JAGS
# Only pass training data for flu/rsv/covid
jags_data <- list(
  flu = data_counts$flu_count[1:t_train],
  rsv = data_counts$rsv_count[1:t_train],
  covid = data_counts$covid_count[1:t_train],
  t_train = t_train,
  tmax = tmax
)
