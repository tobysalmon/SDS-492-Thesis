...
model {
  ...
  for(i in 1:tmax) {
    ...
    rsv[i]   ~ dnegbin(p2[i], r[2]). ## perhaps poisson would be better here
    ...
    lambda4[i] <- exp(phi[i,4]) # multiply by the population of the time (num_patients) to adjust for the sample size shift
  }
...
cat("RSV:   Median =", round(median(samples_combined[, "r[2]"]), 3),
    " | Mean =", round(mean(samples_combined[, "r[2]"]), 3), "\n")
