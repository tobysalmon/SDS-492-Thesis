...
# Kinsa modeled independently; its latent phi[,4] informs disease means.
...
model {
  ...
  # Kinsa (k=4) modeled independently
  mu0[4] ~ dnorm(0, 1e-4)
...
.m2_kinsa_model_file <- file.path(tempdir(), "model2_kinsa_to_diseases.jags")
writeLines(.m2_kinsa_model_string, .m2_kinsa_model_file)
...
fit_m2_kinsa <- function(data_with_pop,
  ...
  run_jags <- function() {
    jm <- jags.model(
      file = .m2_kinsa_model_file, data = jags_data,
  ...
  out <- map_dfr(seq_len(n_holdout_specific), function(h) {
    ...
    map_dfr(seq_len(nrow(disease_info)), function(r) {
      ...
      tibble(
        model = "m2_kinsa", as_of = as_of_date, target_date = target_date,
