# Base ILI nowcast model as a callable function.
# Same structure as fit_base_kinsa but the 4th signal is ILI (negbin with
# num_patients exposure offset) instead of Kinsa (lognormal).
...
.base_ili_model_file <- file.path(tempdir(), "base_model_ili.jags")
writeLines(.base_ili_model_string, .base_ili_model_file)


fit_base_ili <- function(data_with_pop,
                         as_of_date,
  ...
  run_jags <- function() {
    jm <- jags.model(
      file = .base_ili_model_file, data = jags_data,
  ...
  out <- map_dfr(seq_len(n_holdout_specific), function(h) {
    ...
    map_dfr(seq_len(nrow(disease_info)), function(r) {
      ...
      tibble(
        model = "base_ili", as_of = as_of_date, target_date = target_date,
