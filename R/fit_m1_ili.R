# Model 1 (ILI): Specific Diseases -> ILI
# ILI's log-scale mean depends on disease log-rates via b[1:3].
...
.m1_ili_model_file <- file.path(tempdir(), "model1_diseases_to_ili.jags")
writeLines(.m1_ili_model_string, .m1_ili_model_file)


fit_m1_ili <- function(data_with_pop,
                       as_of_date,
  ...
  run_jags <- function() {
    jm <- jags.model(
      file = .m1_ili_model_file, data = jags_data,
  ...
  out <- map_dfr(seq_len(n_holdout_specific), function(h) {
    ...
    map_dfr(seq_len(nrow(disease_info)), function(r) {
      ...
      tibble(
        model = "m1_ili", as_of = as_of_date, target_date = target_date,
